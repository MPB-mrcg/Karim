...
//' @export
// [[Rcpp::export]]
CharacterMatrix isolatePair(CharacterVector fid, CharacterVector iid) {
  int number_isolates = fid.size();
  CharacterMatrix isolate_pairs(number_isolates*number_isolates,4);
  int k = 0;
  
  for (int i = 0; i < (number_isolates); i++) {
    for(int j = 0; j < number_isolates; j++){
